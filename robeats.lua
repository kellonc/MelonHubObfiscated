...
local Window = Library.CreateLib("MelonWare | RoBeats", "DarkTheme")
local Tab = Window:NewTab("Main")
local Section = Tab:NewSection("Auto Player")
_G.atp = false
Section:NewLabel('"A", "S", "D", "F" --USE THESE BINDS LEFT TO RIGHT')
Section:NewToggle("Auto Play", "toggles auto play", function(state)
    if state then
        _G.atp = true
    else
        _G.atp = false
...
Section:NewKeybind("Toggle UI", "toggles the gui", Enum.KeyCode.RightControl, function()
	Library:ToggleUI()
end)
local Autoplayer = {
    noteY = 3879,
    ...
    if instance.ClassName == "CylinderHandleAdornment" then
        instance:GetPropertyChangedSignal("CFrame"):Connect(function()
            if instance.Transparency == 0 and math.floor(instance.CFrame.Y * 10) == Autoplayer.noteY then
                ...
                if noteLane and _G.atp == true then
